...
app.config['MAIL_USERNAME'] = 'atharvarandive28@gmail.com'
app.config['MAIL_PASSWORD'] = 'ijrfvxvohynnceau'
...
sender = 'atharvarandive28@gmail.com'
...
def video_feed():
	if request.method == "POST":
		...
		# Using global cursor
		results = cursor.execute('INSERT INTO proctoring_log (email, name, test_id, voice_db, img_log, user_movements_updown, user_movements_lr, user_movements_eyes, phone_detection, person_status, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
...
def window_event():
	if request.method == "POST":
		...
		# Using global cursor
		results = cursor.execute('INSERT INTO window_estimation_log (email, test_id, name, window_event, uid) values(%s,%s,%s,%s,%s)', (dict(session)['email'], testid, dict(session)['name'], 1, dict(session)['uid']))
		db.commit()
		if(results > 0):
...
def livemonitoringtid():
	# Using global cursor
	results = cursor.execute('SELECT * from teachers where email = %s and uid = %s and proctoring_type = 1', (session['email'], session['uid']))
	if results > 0:
...
def success():
	# Using global cursor
	cursor.execute('UPDATE users set examcredits = examcredits+10 where email = %s and uid = %s', (session['email'], session['uid']))
	db.commit()
...
def payment():
	# Using global cursor
	cursor.execute('SELECT examcredits FROM USERS where email = %s and uid = %s', (session['email'], session['uid']))
	callresults = cursor.fetchone()
...
def lostpassword():
	if request.method == 'POST':
		...
		# Using global cursor
		results = cursor.execute('SELECT * from users where email = %s' , [lpemail])
		if results > 0:
...
def lpnewpwd():
	if request.method == 'POST':
		...
		if(npwd == cpwd ):
			# Using global cursor
			cursor.execute('UPDATE users set password = %s where email = %s', (npwd, slpemail))
			db.commit()
			session.clear()
...
def login():
	if request.method == 'POST':
		...
		# Using global cursor
		results1 = cursor.execute('SELECT uid, name, email, password, user_type, user_image from users where email = %s and user_type = %s and user_login = 0' , (email,user_type))
		if results1 > 0:
			cresults = cursor.fetchone()
			...
			if img_result["verified"] == True and password == password_candidate:
				results2 = cursor.execute('UPDATE users set user_login = 1 where email = %s' , [email])
				db.commit()
				if results2 > 0:
...
def verifyEmail():
	if request.method == 'POST':
		...
		if(theOTP == mOTP):
			try:
				# Using global cursor
				cursor.execute('INSERT INTO users(name, email, password, user_type, user_image, user_login) values(%s,%s,%s,%s,%s,%s)', (dbName, dbEmail, dbPassword, dbUser_type, dbImgdata,0))
				db.commit()
...
def changePassword():
	if request.method == "POST":
		...
		# Using global cursor
		results = cursor.execute('SELECT * from users where email = %s and uid = %s', (session['email'], session['uid']))
		if results > 0:
			data = cursor.fetchone()
			password = data['password']
			usertype = data['user_type']
			if(password == oldPassword):
				cursor.execute("UPDATE users SET password = %s WHERE email = %s", (newPassword, session['email']))
				db.commit()
...
def logout():
	# Using global cursor
	lbr = cursor.execute('UPDATE users set user_login = 0 where email = %s and uid = %s',(session['email'],session['uid']))
	db.commit()
	if lbr > 0:
		session.clear()
		return "success"
...
def examcreditscheck():
	# Using global cursor
	results = cursor.execute('SELECT examcredits from users where examcredits >= 1 and email = %s and uid = %s', (session['email'], session['uid']))
	if results > 0:
		return True
...
def create_test_lqa():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			for row in df.index:
				cursor.execute('INSERT INTO longqa(test_id,qid,q,marks,uid) values(%s,%s,%s,%s,%s)', (test_id, df['qid'][row], df['q'][row], df['marks'][row], session['uid']))
			...
			cursor.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc, proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
			...
			db.commit()
			cursor.execute('UPDATE users SET examcredits = examcredits-1 where email = %s and uid = %s', (session['email'],session['uid']))
			db.commit()
...
def create_test():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			for row in df.index:
				cursor.execute('INSERT INTO questions(test_id,qid,q,a,b,c,d,ans,marks,uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', (test_id, df['qid'][row], df['q'][row], df['a'][row], df['b'][row], df['c'][row], df['d'][row], df['ans'][row], df['marks'][row], session['uid']))
				cur.connection.commit()
...
def create_test_pqa():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			...
			# Using global cursor
			cursor.execute('INSERT INTO practicalqa(test_id,qid,q,compiler,marks,uid) values(%s,%s,%s,%s,%s,%s)', (test_id, 1, questionprac, compiler, marksprac, session['uid']))
			db.commit()
			start_date = form.start_date.data
			end_date = form.end_date.data
			start_time = form.start_time.data
...
def deltidlist():
	# Using global cursor
	results = cursor.execute('SELECT * from teachers where email = %s and uid = %s', (session['email'], session['uid']))
	if results > 0:
		cresults = cursor.fetchall()
		now = datetime.now()
		now = now.strftime("%Y-%m-%d %H:%M:%S")
...
def deldispques():
	if request.method == 'POST':
		...
		if et['test_type'] == "objective":
			# Using global cursor
			cursor.execute('SELECT * from questions where test_id = %s and uid = %s', (tidoption,session['uid']))
			...
			# Using global cursor
			cursor.execute('SELECT * from longqa where test_id = %s and uid = %s', (tidoption,session['uid']))
			callresults = cursor.fetchall()
			return render_template("deldispquesLQA.html", callresults = callresults, tid = tidoption)
		elif et['test_type'] == "practical":
			# Using global cursor
			cursor.execute('SELECT * from practicalqa where test_id = %s and uid = %s', (tidoption,session['uid']))
			callresults = cursor.fetchall()
			...
			if testqdel:
				if ',' in testqdel:
					...
					for getid in testqdel:
						cursor.execute('DELETE FROM questions WHERE test_id = %s and qid =%s and uid = %s', (testid,getid,session['uid']))
						db.commit()
					resp = jsonify('<span style=\'color:green;\'>Questions deleted successfully</span>')
					resp.status_code = 200
					return resp
				else:
					cursor.execute('DELETE FROM questions WHERE test_id = %s and qid =%s and uid = %s', (testid,testqdel,session['uid']))
					db.commit()
					resp = jsonify('<span style=\'color:green;\'>Questions deleted successfully</span>')
					resp.status_code = 200
					return resp
		...
		if request.method == 'POST':
			...
			if testqdel:
				if ',' in testqdel:
					testqdel = testqdel.split(',')
					for getid in testqdel:
						cursor.execute('DELETE FROM longqa WHERE test_id = %s and qid =%s and uid = %s', (testid,getid,session['uid']))
						db.commit()
					resp = jsonify('<span style=\'color:green;\'>Questions deleted successfully</span>')
					resp.status_code = 200
					return resp
				else:
					cursor.execute('DELETE FROM longqa WHERE test_id = %s and qid =%s and uid = %s', (testid,testqdel,session['uid']))
					db.commit()
					resp = jsonify('<span style=\'color:green;\'>Questions deleted successfully</span>')
					resp.status_code = 200
					return resp
		...
		if request.method == 'POST':
			...
			if testqdel:
				if ',' in testqdel:
					testqdel = testqdel.split(',')
					for getid in testqdel:
						cursor.execute('DELETE FROM practicalqa WHERE test_id = %s and qid =%s and uid = %s', (testid,getid,session['uid']))
						db.commit()
					resp = jsonify('<span style=\'color:green;\'>Questions deleted successfully</span>')
					resp.status_code = 200
					return resp
...
def del_qid(testid, qid):
	# Using global cursor
	results = cursor.execute('DELETE FROM questions where test_id = %s and qid = %s and uid = %s', (testid,qid,session['uid']))
	db.commit()
	if results>0:
...
def update_quiz(testid, qid):
	if request.method == 'GET':
		# Using global cursor
		cursor.execute('SELECT * FROM questions where test_id = %s and qid =%s and uid = %s', (testid,qid,session['uid']))
	...
	if request.method == 'POST':
		...
		# Using global cursor
		cursor.execute('UPDATE questions SET q = %s, a = %s, b = %s, c = %s, d = %s, ans = %s, marks = %s where test_id = %s and qid = %s and uid = %s', (ques,ao,bo,co,do,anso,markso,testid,qid,session['uid']))
		cur.connection.commit()
...
def update_lqa(testid, qid):
	if request.method == 'GET':
		# Using global cursor
		cursor.execute('SELECT * FROM longqa where test_id = %s and qid =%s and uid = %s', (testid,qid,session['uid']))
		uresults = cursor.fetchall()
		db.commit()
	...
	if request.method == 'POST':
		...
		markso = request.form['mko']
		# Using global cursor
		cursor.execute('UPDATE longqa SET q = %s, marks = %s where test_id = %s and qid = %s and uid = %s', (ques,markso,testid,qid,session['uid']))
		cur.connection.commit()
		flash('Updated successfully.', 'success')
		return redirect(url_for('updatetidlist'))
	else:
...
def update_PQA(testid, qid):
	if request.method == 'GET':
		# Using global cursor
		cursor.execute('SELECT * FROM practicalqa where test_id = %s and qid =%s and uid = %s', (testid,qid,session['uid']))
		uresults = cursor.fetchall()
		db.commit()
	...
	if request.method == 'POST':
		...
		markso = request.form['mko']
		# Using global cursor
		cursor.execute('UPDATE practicalqa SET q = %s, marks = %s where test_id = %s and qid = %s and uid = %s', (ques,markso,testid,qid,session['uid']))
		cur.connection.commit()
		flash('Updated successfully.', 'success')
		return redirect(url_for('updatetidlist'))
	else:
...
def viewquestions():
	# Using global cursor
	results = cursor.execute('SELECT test_id from teachers where email = %s and uid = %s', (session['email'],session['uid']))
	if results > 0:
		cresults = cursor.fetchall()
...
def examtypecheck(tidoption):
	# Using global cursor
	cursor.execute('SELECT test_type from teachers where test_id = %s and email = %s and uid = %s', (tidoption,session['email'],session['uid']))
	callresults = cursor.fetchone()
	return callresults
...
def viewstudentslogs():
	# Using global cursor
	results = cursor.execute('SELECT test_id from teachers where email = %s and uid = %s and proctoring_type = 0', (session['email'], session['uid']))
	if results > 0:
		cresults = cursor.fetchall()
...
def insertmarkstid():
	# Using global cursor
	results = cursor.execute('SELECT * from teachers where show_ans = 0 and email = %s and uid = %s and (test_type = %s or test_type = %s)', (session['email'], session['uid'],"subjective","practical"))
	if results > 0:
		cresults = cursor.fetchall()
		now = datetime.now()
		now = now.strftime("%Y-%m-%d %H:%M:%S")
...
def displaystudentsdetails():
	if request.method == 'POST':
		tidoption = request.form['choosetid']
		# Using global cursor
		cursor.execute('SELECT DISTINCT email,test_id from proctoring_log where test_id = %s', [tidoption])
		callresults = cursor.fetchall()
...
def insertmarksdetails():
	if request.method == 'POST':
		...
		if et['test_type'] == "subjective":
			# Using global cursor
			cursor.execute('SELECT DISTINCT email,test_id from longtest where test_id = %s', [tidoption])
			callresults = cursor.fetchall()
			return render_template("subdispstudentsdetails.html", callresults = callresults)
		elif et['test_type'] == "practical":
			# Using global cursor
			cursor.execute('SELECT DISTINCT email,test_id from practicaltest where test_id = %s', [tidoption])
			callresults = cursor.fetchall()
...
def insertsubmarks(testid,email):
	if request.method == "GET":
		# Using global cursor
		cursor.execute('SELECT l.email as email, l.marks as inputmarks, l.test_id as test_id, l.qid as qid, l.ans as ans, lqa.marks as marks, l.uid as uid, lqa.q as q  from longtest l, longqa lqa where l.test_id = %s and l.email = %s and l.test_id = lqa.test_id and l.qid = lqa.qid ORDER BY qid ASC', (testid, email))
		callresults = cursor.fetchall()
		return render_template("insertsubmarks.html", callresults = callresults)
	if request.method == "POST":
		# Using global cursor
		results1 = cursor.execute('SELECT COUNT(qid) from longtest where test_id = %s and email = %s',(testid, email))
		results1 = cursor.fetchone()
		for sa in range(1,results1['COUNT(qid)']+1):
			marksByProfessor = request.form[str(sa)]
			# Using global cursor
			cursor.execute('UPDATE longtest SET marks = %s WHERE test_id = %s and email = %s and qid = %s', (marksByProfessor, testid, email, sa))
			db.commit()
...
def displaywinstudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT * from window_estimation_log where test_id = %s and email = %s and window_event = 1', (testid, email))
	callresults = cursor.fetchall()
	return callresults

def countwinstudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT COUNT(*) as wincount from window_estimation_log where test_id = %s and email = %s and window_event = 1', (testid, email))
	callresults = cursor.fetchall()
...
def countMobStudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT COUNT(*) as mobcount from proctoring_log where test_id = %s and email = %s and phone_detection = 1', (testid, email))
	callresults = cursor.fetchall()
...
def countMTOPstudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT COUNT(*) as percount from proctoring_log where test_id = %s and email = %s and person_status = 1', (testid, email))
	callresults = cursor.fetchall()
...
def countTotalstudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT COUNT(*) as total from proctoring_log where test_id = %s and email = %s', (testid, email))
	callresults = cursor.fetchall()
...
def displaystudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT * from proctoring_log where test_id = %s and email = %s', (testid, email))
	callresults = cursor.fetchall()
...
def mobdisplaystudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT * from proctoring_log where test_id = %s and email = %s and phone_detection = 1', (testid, email))
	callresults = cursor.fetchall()
...
def persondisplaystudentslogs(testid,email):
	# Using global cursor
	cursor.execute('SELECT * from proctoring_log where test_id = %s and email = %s and person_status = 1', (testid, email))
	callresults = cursor.fetchall()
...
def share_details(testid,email):
	# Using global cursor
	cursor.execute('SELECT * from teachers where test_id = %s and email = %s', (testid, email))
	callresults = cursor.fetchall()
...
def publish_results_testid():
	# Using global cursor
	results = cursor.execute('SELECT * from teachers where test_type != %s AND show_ans = 0 AND email = %s AND uid = %s', ("objectve", session['email'], session['uid']))
	if results > 0:
		cresults = cursor.fetchall()
		now = datetime.now()
		now = now.strftime("%Y-%m-%d %H:%M:%S")
...
def viewresults():
	if request.method == 'POST':
		...
		if et['test_type'] == "subjective":
			# Using global cursor
			cursor.execute('SELECT SUM(marks) as marks, email from longtest where test_id = %s group by email', ([tidoption]))
			callresults = cursor.fetchall()
			return render_template("publish_viewresults.html", callresults = callresults, tid = tidoption)
		elif et['test_type'] == "practical":
			# Using global cursor
			cursor.execute('SELECT SUM(marks) as marks, email from practicaltest where test_id = %s group by email', ([tidoption]))
			callresults = cursor.fetchall()
			return render_template("publish_viewresults.html", callresults = callresults, tid = tidoption)
		else:
			flash("Some Error Occured!")
...
def publish_results():
	if request.method == 'POST':
		...
		# Using global cursor
		cursor.execute('UPDATE teachers set show_ans = 1 where test_id = %s', ([tidoption]))
		db.commit()
...
def test_update_time():
	if request.method == 'POST':
		...
		testid = request.form['testid']
		cursor.execute('UPDATE studentTestInfo set time_left=SEC_TO_TIME(%s) where test_id = %s and email = %s and uid = %s and completed=0', (time_left, testid, session['email'], session['uid']))
		db.commit()
		t1 = cur.rowcount
		if t1 > 0:
			...
			# Using global cursor
			cursor.execute('INSERT into studentTestInfo (email, test_id,time_left,uid) values(%s,%s,SEC_TO_TIME(%s),%s)', (session['email'], testid, time_left, session['uid']))
			t2 = db.commit()
			t2 = cur.rowcount
			if t2 > 0:
	...
	if request.method == 'POST' and form.validate():
		...
		if results1 > 0:
			...
			if img_result["verified"] == True:
				# Using global cursor
				results = cursor.execute('SELECT * from teachers where test_id = %s', [test_id])
				if results > 0:
					data = cursor.fetchone()
					password = data['password']
					...
					if password == password_candidate:
						...
						if datetime.strptime(start,"%Y-%m-%d %H:%M:%S") < now and datetime.strptime(end,"%Y-%m-%d %H:%M:%S") > now:
							results = cursor.execute('SELECT time_to_sec(time_left) as time_left,completed from studentTestInfo where email = %s and test_id = %s', (session['email'], test_id))
							if results > 0:
								...
								if is_completed == 0:
									...
									if time_left <= duration:
										duration = time_left
										results = cursor.execute('SELECT qid , ans from students where email = %s and test_id = %s and uid = %s', (session['email'], test_id, session['uid']))
										marked_ans = {}
										if results > 0:
								...
								cursor.execute('INSERT into studentTestInfo (email, test_id,time_left,uid) values(%s,%s,SEC_TO_TIME(%s),%s)', (session['email'], test_id, duration, session['uid']))
								db.commit()
								results = cursor.execute('SELECT time_to_sec(time_left) as time_left,completed from studentTestInfo where email = %s and test_id = %s and uid = %s', (session['email'], test_id, session['uid']))
								if results > 0:
									results = cursor.fetchone()
									is_completed = results['completed']
									if is_completed == 0:
										...
										if time_left <= duration:
											duration = time_left
											results = cursor.execute('SELECT * from students where email = %s and test_id = %s and uid = %s', (session['email'], test_id, session['uid']))
											marked_ans = {}
											if results > 0:
												results = cursor.fetchall()
												for row in results:
...
def test(testid):
	# Using global cursor
	cursor.execute('SELECT test_type from teachers where test_id = %s ', [testid])
	callresults = cursor.fetchone()
	if callresults['test_type'] == "objective":
		...
		if request.method == 'GET':
			...
			if flag == 'get':
				num = request.form['no']
				results = cursor.execute('SELECT test_id,qid,q,a,b,c,d,ans,marks from questions where test_id = %s and qid =%s',(testid, num))
				if results > 0:
					data = cursor.fetchone()
					del data['ans']
				...
				# Using global cursor
				results = cursor.execute('SELECT * from students where test_id =%s and qid = %s and email = %s', (testid, qid, session['email']))
				if results > 0:
					cursor.execute('UPDATE students set ans = %s where test_id = %s and qid = %s and email = %s', (testid, qid, session['email']))
					db.commit()
				else:
					cursor.execute('INSERT INTO students(email,test_id,qid,ans,uid) values(%s,%s,%s,%s,%s)', (session['email'], testid, qid, ans, session['uid']))
					db.commit()
			elif flag=='time':
				# Using global cursor
				time_left = request.form['time']
				...
				# Using global cursor
				cursor.execute('UPDATE studentTestInfo set completed=1,time_left=sec_to_time(0) where test_id = %s and email = %s and uid = %s', (testid, session['email'],session['uid']))
				db.commit()
		...
		if request.method == 'GET':
			# Using global cursor
			cursor.execute('SELECT test_id, qid, q, marks from longqa where test_id = %s ORDER BY RAND()',[testid])
			callresults1 = cursor.fetchall()
			cursor.execute('SELECT time_to_sec(time_left) as duration from studentTestInfo where completed = 0 and test_id = %s and email = %s and uid = %s', (testid, session['email'], session['uid']))
			studentTestInfo = cursor.fetchone()
			if studentTestInfo != None:
				duration = studentTestInfo['duration']
				cursor.execute('SELECT test_id, subject, topic, proctoring_type from teachers where test_id = %s',[testid])
				testDetails = cursor.fetchone()
				...
				# Using global cursor
				cursor.execute('SELECT test_id, duration, subject, topic from teachers where test_id = %s',[testid])
				testDetails = cursor.fetchone()
				subject = testDetails['subject']
			...
			# Using global cursor
			results1 = cursor.execute('SELECT COUNT(qid) from longqa where test_id = %s',[testid])
			results1 = cursor.fetchone()
			insertStudentData = None
			for sa in range(1,results1['COUNT(qid)']+1):
				answerByStudent = request.form[str(sa)]
				# Using global cursor
				insertStudentData = cursor.execute('INSERT INTO longtest(email,test_id,qid,ans,uid) values(%s,%s,%s,%s,%s)', (session['email'], testid, sa, answerByStudent, session['uid']))
				db.commit()
			else:
				if insertStudentData > 0:
					insertStudentTestInfoData = cursor.execute('UPDATE studentTestInfo set completed = 1 where test_id = %s and email = %s and uid = %s', (test_id, session['email'], session['uid']))
					db.commit()
		...
		if request.method == 'GET':
			# Using global cursor
			cursor.execute('SELECT test_id, qid, q, marks, compiler from practicalqa where test_id = %s ORDER BY RAND()',[testid])
			callresults1 = cursor.fetchall()
			cursor.execute('SELECT time_to_sec(time_left) as duration from studentTestInfo where completed = 0 and test_id = %s and email = %s and uid = %s', (testid, session['email'], session['uid']))
			studentTestInfo = cursor.fetchone()
			if studentTestInfo != None:
			...
			# Using global cursor
			insertStudentData = cursor.execute('INSERT INTO practicaltest(email,test_id,qid,code,input,executed,uid) values(%s,%s,%s,%s,%s,%s,%s)', (session['email'], testid, "1", codeByStudent, inputByStudent, executedByStudent, session['uid']))
			db.commit()
			if insertStudentData > 0:
				insertStudentTestInfoData = cursor.execute('UPDATE studentTestInfo set completed = 1 where test_id = %s and email = %s and uid = %s', (test_id, session['email'], session['uid']))
				db.commit()
...
def random_gen():
	if request.method == "POST":
		id = request.form['id']
		# Using global cursor
		results = cursor.execute('SELECT count(*) from questions where test_id = %s', [id])
		if results > 0:
			data = cursor.fetchone()
			total = data['count(*)']
...
def check_result(email, testid):
	if email == session['email']:
		# Using global cursor
		results = cursor.execute('SELECT * FROM teachers where test_id = %s', [testid])
		if results>0:
			results = cursor.fetchone()
			check = results['show_ans']
			if check == 1:
				results = cursor.execute('select q,a,b,c,d,marks,q.qid as qid, \
...
def neg_marks(email,testid,negm):
	...
	results = cursor.execute("select marks,q.qid as qid, \
...
def totmarks(email,tests): 
	...
	for test in tests:
		testid = test['test_id']
		results=cursor.execute("select neg_marks from teachers where test_id=%s",[testid])
		results=cursor.fetchone()
...
def tests_given(email):
	if request.method == "GET":
		if email == session['email']:
			# Using global cursor
			resultsTestids = cursor.execute('select studenttestinfo.test_id as test_id from studenttestinfo,teachers where studenttestinfo.email = %s and studenttestinfo.uid = %s and studenttestinfo.completed=1 and teachers.test_id = studenttestinfo.test_id and teachers.show_ans = 1 ', (session['email'], session['uid']))
			resultsTestids = cursor.fetchall()
...
def tests_created(email):
	if email == session['email']:
		# Using global cursor
		results = cursor.execute('select * from teachers where email = %s and uid = %s and show_ans = 1', (email,session['uid']))
		results = cursor.fetchall()
...
def student_results(email, testid):
	if email == session['email']:
		...
		if request.method =='GET':
			if et['test_type'] == "objective":
				# Using global cursor
				results = cursor.execute('select users.name as name,users.email as email, studentTestInfo.test_id as test_id from studentTestInfo, users where test_id = %s and completed = 1 and  users.user_type = %s and studentTestInfo.email=users.email ', (testid,'student'))
				results = cursor.fetchall()
				final = []
				names = []
				scores = []
				...
				# Using global cursor
				results = cursor.execute('select users.name as name,users.email as email, longtest.test_id as test_id, SUM(longtest.marks) AS marks from longtest, users where longtest.test_id = %s  and  users.user_type = %s and longtest.email=users.email', (testid,'student'))
				results = cursor.fetchall()
				names = []
				scores = []
				for user in results:
				...
				# Using global cursor
				results = cursor.execute('select users.name as name,users.email as email, practicaltest.test_id as test_id, SUM(practicaltest.marks) AS marks from practicaltest, users where practicaltest.test_id = %s  and  users.user_type = %s and practicaltest.email=users.email', (testid,'student'))
				results = cursor.fetchall()
				names = []
				scores = []
				for user in results:
...
def disptests(email):
	if email == session['email']:
		# Using global cursor
		results = cursor.execute('select * from teachers where email = %s and uid = %s', (email,session['uid']))
		results = cursor.fetchall()
...
def student_test_history(email):
	if email == session['email']:
		# Using global cursor
		results = cursor.execute('SELECT a.test_id, b.subject, b.topic \
